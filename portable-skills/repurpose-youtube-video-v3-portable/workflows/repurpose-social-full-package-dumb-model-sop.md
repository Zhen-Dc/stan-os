...
AIS-OS/workflows/client-trust-linkedin-scrape-and-recreate-sop.md
...
- Creator row with `Stanley Chima`, verified badge, and the current approved
  username. As of 2026-07-14, render the username as
  `chima-stanley-chuku` and store the LinkedIn URL separately in metadata:
  `https://www.linkedin.com/in/chima-stanley-chuku/`.
...
1. Use public text, screenshots, pasted text, or saved source briefs.
2. If the full post is blocked, do not pretend it was scraped.
3. Mark the package `source-brief-derived` if only the local source brief is
   available.
4. Recreate the strategy for Stanley. Do not copy the creator's wording.
...
The GIF only changes one thing: it highlights one row per frame.
...
- Use `Stanley Chima` as the visible name.
- Use `@chima-stanley-chuku` as the visible username unless Stanley gives a new
  username. Store `https://www.linkedin.com/in/chima-stanley-chuku/` in JSON
  metadata as `profile_url`; do not render Markdown link syntax on the image.
...
- For slides 2-8, move the first body section close to the title area. Do not
...
- Use a large title.
- Add more subtext than the earlier weak version.
- Use the white space with useful preview text, chips, mini benefit rows, or a
  small "what you will learn" block.
...
- Do not reuse the same stacked-card teaching layout from slides 2-8 for the
  cover. The cover needs its own editorial layout: strong tool/topic badge,
  large hook, compact profile pill with Stanley's real brand avatar, one clear
  rule or promise panel, and a clean footer area for page number plus next-slide
  icon.
...
- Do not render production labels such as `COVER PROMISE` on the public cover.
  Use audience-facing labels like `INSIDE`, `SAVE THIS`, or the actual tool tag.
  Recheck support cards so no words are clipped by borders.
...
2. Read this SOP.
3. Read `repurpose-youtube-video-execution-sop.md`.
4. Read platform rules and visual recipes.
5. Identify whether the source is YouTube, LinkedIn, script, transcript, or
   source brief.
6. Capture the source honestly.
...
18. Confirm LinkedIn static image and GIF have the same content.
19. Render Instagram carousel.
20. Confirm slide 1 has stronger subtext and slides 2-8 vary.
21. Render X cards.
22. Confirm every X card stands alone.
23. Render Facebook visuals.
24. Create `preview/preview.md`.
25. Create `preview/image-qa.md`.
26. Update `manifest.json`.
27. Count media.
28. Inspect representative images.
29. Fix any layout, clipping, ambiguity, repeated module, or blank-space issue.
30. Rerender after fixes.
31. Ask the user what to approve next.
...
- Does LinkedIn GIF highlight one row per frame?
- Does Instagram slide 1 have enough subtext?
- Do Instagram slides 2-8 use varied layouts?
