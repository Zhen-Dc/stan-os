...
replace rendering. V3 still renders the actual PNG files and LinkedIn GIF.
...
- `AIS-OS/workflows/repurpose-youtube-video-execution-sop.md` before executing the workflow end to end.
  For LinkedIn/social post repurposing, follow its `Dumb-Model Runbook:
  LinkedIn/Social Post Repurpose` section exactly.
- `AIS-OS/workflows/repurpose-social-full-package-dumb-model-sop.md` before
  generating or repairing visuals. This is the locked Stanley template SOP:
  standalone LinkedIn image/GIF content, dense Instagram slides, varied module
  labels, standalone X cards, and full package QA.
- `AIS-OS/workflows/client-trust-linkedin-scrape-and-recreate-sop.md` first when the source is a LinkedIn/social post or a reference post to recreate.
...
```
   ...
   the subtitle, and one orange highlighted phrase. Use `Stanley Chima` as the
   name, `chima-stanley-chuku` as the visible username, and store
   `https://www.linkedin.com/in/chima-stanley-chuku/` as `profile_url` metadata
