...
- When Stanley provides a LinkedIn/social/blog/article/Substack/web URL and says "repurpose", "recreate", or asks for content from it, use the v3 repurpose flow. Capture the web source with Firecrawl first, then route through `workflows/client-trust-linkedin-scrape-and-recreate-sop.md` when source analysis is needed and `workflows/repurpose-youtube-video-execution-sop.md` for the full social package unless he explicitly asks for analysis only.
