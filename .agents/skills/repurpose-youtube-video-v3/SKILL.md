...
- `AIS-OS/workflows/repurpose-social-full-package-dumb-model-sop.md` is the
  ...
  handoff.
- `.agents/skills/repurpose-post-qa-agent/SKILL.md` is the mandatory final QA
  gate. Use it after final rendering and after every correction that changes
  public copy or media.
...
not draft until that visual is captured or honestly marked blocked. Before
drafting from LinkedIn captures, reject unrelated saved images. Public LinkedIn
HTML can include profile photos, repost thumbnails, side-feed images, or
unrelated people-only images. Keep only feedshare/carousel assets whose visible
topic matches the caption/body, and document rejected assets in
...
4. Draft editable plain text files in each platform text folder:
   ...
   - Captions must be structured before approval. For Instagram and Facebook,
     start with a strong hook, then the reason it matters now, the useful body
     or framework, proof/data/checkable detail, and one CTA. Keep the caption about
     70% similar to the reference post caption's structure and core content:
     preserve the same sequence, claim, examples, proof, and ask where safe,
     then paraphrase into Stanley's voice. Do not submit captions that are only
     loose notes, summaries, or a generic "save this" ending.
     When a keyword CTA exists, it must be the final ask at the end of the
     caption, written as a natural sentence. Do not place a naked keyword or
     `Comment KEYWORD.` line before the caption.
...
```
   ...
   the subtitle, and one orange highlighted phrase. Use `stanley chima`
   as the display name, `@chima-stanley-chukwu` as the visible username, and store
...
For every new post, analyze both the caption/body and attached image before
building. The image can define the correct slide system: decision map, effort
dial, cost ladder, matrix, receipt, workflow loop, console, audit, or tree. Do
not reuse the previous post's visual structures when the new source has a
different shape. Repeated project-level structure is a QA failure.

Cover titles must be gigantic, not merely large. If the automatic cover layout
shrinks the headline, force 1-3 title lines and a cover font size around 190px
or higher on 1080x1440, using the accepted `Claude+Higgsfield Ai Agent` cover as
the visual benchmark.

Cover subtext must stay readable. When the cover needs summary context, use 3-4
short lines that explain the post clearly; do not shrink it into a tiny caption.
On final QA, check every slide for low-contrast colored text on light/white
panels, incomplete sentences, clipped text, and text leaving its diagram box.
Open flagged slides individually after viewing the contact sheet.
...
Final QA is the last step for every product before handoff. After rendering and
visual inspection, tell Stanley honestly whether the finished cover/post is
likely to stop the target audience or make them keep scrolling, what they will
think or feel first, and why. Do not give this audience judgment before seeing
the finished visuals.

Run the dedicated QA agent before approval or Drive delivery:
...
Then apply the full editorial and visual rubric in
`.agents/skills/repurpose-post-qa-agent/references/qa-rubric.md` and write
`preview/post-qa-agent-report.md`. A package with a hard fail, a score below
90, or a `MIXED`/`LIKELY TO SCROLL` audience verdict is not finished.
