...
| `workflows/` | You want a repeatable process the AIOS can run end to end | Markdown SOPs for WAT workflows: objective, inputs, tools, outputs, and edge cases |
...
| `tools/` | You need deterministic execution for APIs, scraping, transformations, or repeatable file work | Scripts that make execution reliable while the AIOS handles reasoning and coordination |
...
- `workflows/` — when a process should become an end-to-end WAT workflow the AIOS can run
- `tools/` — when a repeated execution step should be handled by deterministic code
