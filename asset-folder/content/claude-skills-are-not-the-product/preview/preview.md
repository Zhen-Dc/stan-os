# Preview - Claude Skills Are Not The Product

## Source

- URL: https://www.linkedin.com/feed/update/urn:li:activity:7477290928815169536/
- Source confidence: public-post-derived
- Source brief: `source/source-brief.md`
- Source capture: `source/transcript.txt`

## Draft text files

- LinkedIn: `drafts/LinkedIn/text/post.txt`
- Instagram carousel: `drafts/Instagram/text/post.txt`
- Instagram caption: `drafts/Instagram/text/caption.txt`
- X post: `drafts/X/text/post.txt`
- X thread: `drafts/X/text/thread.txt`
- Facebook post: `drafts/Facebook/text/post.txt`
- Facebook caption: `drafts/Facebook/text/caption.txt`

## Visual prompt files

- `drafts/LinkedIn/images/thumbnail-prompt.txt`
- `drafts/LinkedIn/images/visual-prompt.txt`
- `drafts/Instagram/images/slide-01-cover-prompt.txt`
- `drafts/Instagram/images/thumbnail-prompt.txt`
- `drafts/Instagram/images/visual-prompt.txt`
- `drafts/X/images/thumbnail-prompt.txt`
- `drafts/X/images/visual-prompt.txt`
- `drafts/Facebook/images/thumbnail-prompt.txt`
- `drafts/Facebook/images/visual-prompt.txt`

## Generated PNG files

- `drafts/LinkedIn/images/linkedin-thumbnail.png`
- `drafts/LinkedIn/images/linkedin-diagram.png`
- `drafts/Instagram/images/instagram-slide-01.png`
- `drafts/Instagram/images/instagram-slide-02.png`
- `drafts/Instagram/images/instagram-slide-03.png`
- `drafts/Instagram/images/instagram-slide-04.png`
- `drafts/Instagram/images/instagram-slide-05.png`
- `drafts/Instagram/images/instagram-slide-06.png`
- `drafts/Instagram/images/instagram-slide-07.png`
- `drafts/Instagram/images/instagram-slide-08.png`
- `drafts/X/images/x-thumbnail.png`
- `drafts/X/images/x-quote-01.png`
- `drafts/X/images/x-quote-02.png`
- `drafts/X/images/x-quote-03.png`
- `drafts/X/images/x-quote-04.png`
- `drafts/X/images/x-quote-05.png`
- `drafts/X/images/x-quote-06.png`
- `drafts/X/images/x-quote.png`
- `drafts/Facebook/images/facebook-thumbnail.png`
- `drafts/Facebook/images/facebook-course-card.png`

## Generated GIF files

- `drafts/LinkedIn/images/linkedin-animation.gif`

## Summary

The package reframes the source post from "download Claude Skills" into Stanley's stronger AI automation angle: prompts are not the asset, repeatable workflows are.

The latest image pass fixes the standalone-content issue: LinkedIn image/GIF share a clearer title, X cards are independent posts, and the Instagram carousel uses denser varied layouts instead of repeating the same prompt-box structure.

The remade images follow the full repurpose package contract: platform folders, 20 PNGs, 1 LinkedIn GIF, prompt files, QA file, and manifest counts.

## Image QA

See `preview/image-qa.md`.

## Approval instruction

Review the drafts and generated images, then approve the platform(s) you want to move into `approved/`.
